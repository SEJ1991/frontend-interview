  ...
  const questions = {
    cs: [
      ...
      },
      {
        ...
        answer:
          "OSI 7계층은 네트워크 통신을 7단계로 나눈 모델로, 물리 계층부터 응용 계층까지 데이터 통신 과정을 정의합니다. 물리 계층은 하드웨어적 연결을, 응용 계층은 최종 사용자와의 상호작용을 다룹니다.",
      },
      {
        id: "cs_13",
        question: "세션과 쿠키의 차이점은 무엇인가요?",
        answer:
          "쿠키는 클라이언트에 저장되는 작은 데이터로, 서버와의 상태를 저장하는 데 사용됩니다. 세션은 서버에 저장되며, 클라이언트와 서버 간의 상태를 유지하기 위해 사용됩니다.",
      },
      {
        id: "cs_14",
        question: "웹소켓(WebSocket)이란 무엇인가요?",
        answer:
          "웹소켓은 클라이언트와 서버 간의 양방향 통신을 가능하게 하는 프로토콜로, 실시간 데이터 전송에 유용합니다. HTTP와 달리 연결이 유지되어 데이터를 주고받을 수 있습니다.",
      },
      {
        id: "cs_15",
        question: "API Gateway란 무엇인가요?",
        answer:
          "API Gateway는 클라이언트 요청을 여러 내부 서비스로 라우팅하는 역할을 합니다. 마이크로서비스 아키텍처에서 주로 사용되며, 인증, 로깅, 로드 밸런싱 등의 기능을 제공합니다.",
      },
      {
        id: "cs_16",
      ...
      },
      {
        ...
        answer:
          "OAuth는 사용자 비밀번호를 노출하지 않고 제3의 애플리케이션이 사용자 데이터를 접근할 수 있도록 인증하는 프로토콜입니다. 주로 소셜 로그인 등에 사용됩니다.",
      },
      {
        id: "cs_18",
      ...
      },
      {
        ...
        answer:
          "JWT는 JSON 형식의 데이터를 사용한 인증 토큰으로, 클라이언트와 서버 간의 정보를 안전하게 전송하기 위한 표준입니다. 주로 인증과 권한 부여에 사용됩니다.",
      },
      {
        id: "cs_20",
        question: "IPv4와 IPv6의 차이점은 무엇인가요?",
        answer:
          "IPv4는 32비트 주소 체계를 사용하여 약 43억 개의 주소를 제공하며, IPv6는 128비트 주소 체계를 사용하여 사실상 무한대의 주소를 제공합니다. IPv6는 IPv4 주소 고갈 문제를 해결하기 위해 도입되었습니다.",
      },
    ],
    javascript: [
      {
        ...
        answer:
          "this는 함수가 호출되는 맥락에 따라 달라집니다. 전역에서 this는 window 객체를 가리키며, 메서드 내부에서는 그 메서드를 소유한 객체를 가리킵니다. 또한 화살표 함수에서 this는 자신을 감싸고 있는 스코프의 this를 그대로 상속받습니다.",
      },
      {
        id: "js_2",
        question: "var, let, const의 차이점은 무엇인가요?",
        answer:
          "var는 함수 스코프를 가지며, 중복 선언이 가능합니다. let과 const는 블록 스코프를 가지며, 중복 선언이 불가능합니다. let은 값을 재할당할 수 있지만, const는 선언과 동시에 값을 할당해야 하고 이후 재할당이 불가능합니다.",
      },
      {
        id: "js_3",
        question: "JavaScript의 비동기 처리 방식에 대해 설명해주세요.",
        answer:
          "JavaScript는 단일 스레드에서 동작하지만, 비동기 처리를 통해 논블로킹 방식으로 작업을 처리합니다. 이를 위해 콜백 함수, 프로미스(Promise), async/await을 사용합니다. 비동기 작업이 완료되면 Event Loop가 Task Queue에서 이를 실행합니다.",
      },
      {
        id: "js_4",
      ...
      },
      {
        ...
        answer:
          "호이스팅은 변수와 함수 선언이 코드의 상단으로 끌어올려지는 JavaScript의 동작 방식입니다. 변수는 선언만 호이스팅되며, 할당은 호이스팅되지 않습니다. 함수 선언은 전체가 호이스팅됩니다.",
      },
      {
        id: "js_7",
        question:
        ...
        answer:
          "이벤트 버블링은 자식 요소에서 발생한 이벤트가 부모 요소로 전파되는 현상이고, 이벤트 캡처링은 부모 요소에서 자식 요소로 이벤트가 전파되는 방식입니다. 이벤트 리스너에 `useCapture` 옵션을 설정하여 캡처링 단계를 제어할 수 있습니다.",
      },
      {
        id: "js_8",
        question:
        ...
        answer:
          "화살표 함수는 자신만의 this를 가지지 않고, 자신이 선언된 위치의 this를 상속받습니다. 반면 일반 함수는 호출될 때마다 자신만의 this를 생성합니다.",
      },
      {
        id: "js_9",
        question: "JavaScript에서 truthy와 falsy 값은 무엇인가요?",
        answer:
          "truthy 값은 조건식에서 true로 평가되는 값이며, falsy 값은 false로 평가되는 값입니다. 대표적인 falsy 값으로는 0, 빈 문자열, null, undefined, NaN 등이 있습니다.",
      },
      {
        id: "js_10",
        question: "spread 연산자와 rest 연산자의 차이점은 무엇인가요?",
        answer:
          "spread 연산자는 배열이나 객체의 값을 개별 요소로 분리하여 사용하고, rest 연산자는 함수의 인자나 배열의 나머지 요소들을 하나로 모아 배열로 반환합니다.",
      },
      {
        id: "js_11",
        question:
        ...
        answer:
          "동기 프로그래밍은 작업이 순차적으로 실행되는 방식이고, 비동기 프로그래밍은 작업이 완료되기를 기다리지 않고 다른 작업을 처리하는 방식입니다. 비동기는 주로 시간이 오래 걸리는 작업에 사용됩니다.",
      },
      {
        id: "js_12",
        question: "콜백 함수(callback function)란 무엇인가요?",
        answer:
          "콜백 함수는 다른 함수에 인자로 전달되어 특정 작업이 완료된 후 호출되는 함수입니다. 비동기 작업을 처리할 때 많이 사용됩니다.",
      },
      {
        id: "js_13",
        question:
        ...
        answer:
          "이벤트 위임은 상위 요소에 이벤트 리스너를 설정하여 하위 요소의 이벤트를 처리하는 기법입니다. DOM 요소가 동적으로 추가될 때에도 이벤트를 처리할 수 있다는 장점이 있습니다.",
      },
      {
        id: "js_14",
        question: "null과 undefined의 차이점은 무엇인가요?",
        answer:
          "null은 명시적으로 값이 비어 있음을 나타내는 값이며, undefined는 변수가 선언되었으나 값이 할당되지 않았을 때 나타나는 값입니다.",
      },
      {
        id: "js_15",
        question: "JavaScript의 모듈 시스템에 대해 설명해주세요.",
        answer:
          "JavaScript의 모듈 시스템은 코드를 모듈화하여 재사용성을 높이고, 의존성을 관리하는 방식입니다. ES6부터 `import`와 `export` 키워드를 사용하여 모듈을 정의하고 가져올 수 있습니다.",
      },
      {
        id: "js_16",
        question: "이벤트 루프(Event Loop)란 무엇인가요?",
        answer:
          "이벤트 루프는 JavaScript의 비동기 작업을 처리하는 메커니즘으로, 콜백 함수나 프로미스가 실행되기를 대기하는 작업을 관리하고 Call Stack이 비어 있을 때 작업을 실행합니다.",
      },
      {
        id: "js_17",
        question: "Promise와 async/await의 차이점은 무엇인가요?",
        answer:
          "Promise는 비동기 작업의 상태와 결과를 처리하는 객체이며, async/await은 Promise를 더 직관적으로 사용할 수 있게 도와주는 문법입니다. async/await은 비동기 작업을 동기식 코드처럼 작성할 수 있게 해줍니다.",
      },
      {
        id: "js_18",
        question:
        ...
        answer:
          "이뮤터블 객체는 변경 불가능한 객체로, 한 번 생성되면 상태가 변하지 않습니다. 반면 뮤터블 객체는 상태를 변경할 수 있습니다. 이뮤터블 객체는 상태 관리를 더 쉽게 만들어 줍니다.",
      },
      {
        id: "js_19",
        question: "객체(Object)와 배열(Array)의 차이점은 무엇인가요?",
        answer:
          "객체는 키-값 쌍으로 데이터를 저장하며, 배열은 순서가 있는 리스트 형태로 데이터를 저장합니다. 배열은 주로 리스트 데이터에, 객체는 서로 관련된 데이터를 그룹화할 때 사용됩니다.",
      },
      {
        id: "js_20",
        question: "Strict mode란 무엇인가요?",
        answer:
          "Strict mode는 JavaScript의 오류를 보다 엄격하게 처리하여 코드의 안정성을 높이는 모드입니다. 'use strict' 지시문을 코드 상단에 추가하여 활성화할 수 있습니다.",
      },
    ],
    react: [
      {
        ...
        answer:
          "state는 컴포넌트 내부에서 관리하는 동적인 데이터로, 상태가 변경되면 컴포넌트가 다시 렌더링됩니다. props는 부모 컴포넌트가 자식 컴포넌트에 전달하는 데이터로, 자식 컴포넌트에서 수정할 수 없습니다.",
      },
      {
        id: "react_2",
        question:
      ...
      },
      {
        ...
        answer:
          "React 컴포넌트는 마운트(mount), 업데이트(update), 언마운트(unmount)의 세 가지 주요 라이프사이클을 가집니다. 이 과정에서 componentDidMount, componentDidUpdate, componentWillUnmount와 같은 라이프사이클 메서드나 useEffect를 통해 특정 작업을 실행할 수 있습니다.",
      },
      {
        id: "react_4",
        question: "useEffect Hook은 어떤 역할을 하나요?",
        answer:
          "useEffect는 컴포넌트가 렌더링될 때마다 특정 작업을 수행할 수 있게 해주는 Hook입니다. 주로 데이터 패칭, DOM 조작, 구독 설정 및 해제를 위해 사용되며, 두 번째 인자인 의존성 배열을 통해 특정 값이 변경될 때만 실행되도록 제어할 수 있습니다.",
      },
      {
        id: "react_5",
        question: "React에서 가상 DOM(Virtual DOM)이란 무엇인가요?",
        answer:
          "가상 DOM은 React가 실제 DOM과 상호작용하기 전에 메모리 내에서 DOM 구조를 추상화한 객체입니다. 변경 사항을 가상 DOM에서 먼저 계산한 후, 실제 DOM에 최소한의 업데이트를 적용하여 성능을 최적화합니다.",
      },
      {
        id: "react_6",
        question:
        ...
        answer:
          "조건부 렌더링은 특정 조건에 따라 다른 컴포넌트를 렌더링하는 방식입니다. JavaScript의 논리 연산자나 삼항 연산자를 사용하여 구현할 수 있습니다.",
      },
      {
        id: "react_7",
        question: "React의 Fragment란 무엇인가요?",
        answer:
          "Fragment는 여러 자식 컴포넌트를 그룹화할 때 불필요한 DOM 노드를 추가하지 않기 위해 사용하는 컴포넌트입니다. 빈 태그(`<></>`)나 React.Fragment를 사용하여 구현할 수 있습니다.",
      },
      {
        id: "react_8",
        question: "React에서 key props는 왜 필요한가요?",
        answer:
          "key는 React가 어떤 항목이 변경, 추가, 또는 제거되었는지 식별하는 데 사용되는 고유한 값입니다. 이를 통해 리스트 렌더링 시 성능을 최적화하고, 중복되는 DOM 업데이트를 방지할 수 있습니다.",
      },
      {
        id: "react_9",
        question: "컴포넌트 간 상태를 어떻게 공유하나요?",
        answer:
          "컴포넌트 간 상태는 상위 컴포넌트에서 상태를 관리하고 props를 통해 하위 컴포넌트로 전달하는 방식으로 공유할 수 있습니다. 또는 Context API를 사용하여 더 깊은 트리 구조에서도 상태를 전달할 수 있습니다.",
      },
      {
        id: "react_10",
      ...
      },
      {
        ...
        answer:
          "Redux는 전역 상태 관리를 위한 도구로, 애플리케이션의 상태를 하나의 중앙 저장소(Store)에서 관리합니다. 상태 변경을 명시적으로 관리하여 예측 가능하고 일관성 있는 상태 관리를 할 수 있습니다.",
      },
      {
        id: "react_12",
        question:
        ...
        answer:
          "액션은 상태를 변경하는 요청을 의미하며, 리듀서는 액션을 처리하여 새로운 상태를 반환하는 함수입니다. 액션은 주로 객체 형태로, 리듀서는 순수 함수로 작성됩니다.",
      },
      {
        id: "react_13",
        question: "useReducer Hook은 언제 사용하나요?",
        answer:
          "useReducer는 useState보다 복잡한 상태 관리 로직이 필요할 때 사용하는 Hook입니다. 리듀서 함수를 통해 상태와 액션을 처리하여 상태를 관리합니다.",
      },
      {
        id: "react_14",
        question: "컴포넌트에서 메모이제이션(Memoization)이란 무엇인가요?",
        answer:
          "메모이제이션은 컴포넌트가 동일한 입력값으로 반복해서 렌더링되지 않도록 값이나 함수의 결과를 캐싱하는 최적화 기법입니다. React에서는 useMemo와 useCallback Hook을 사용하여 메모이제이션을 구현할 수 있습니다.",
      },
      {
        id: "react_15",
        question:
        ...
        answer:
          "서버 사이드 렌더링은 초기 페이지를 서버에서 렌더링하여 클라이언트에 전달하는 방식입니다. 이를 통해 초기 로딩 시간을 줄이고, SEO를 개선할 수 있습니다. 주로 Next.js와 같은 프레임워크에서 사용됩니다.",
      },
      {
        id: "react_16",
        question: "React의 컴포넌트 최적화 기법은 무엇이 있나요?",
        answer:
          "React 컴포넌트 최적화 기법으로는 React.memo, useMemo, useCallback을 사용한 메모이제이션, 가상 DOM을 활용한 효율적인 업데이트, Lazy loading을 통한 코드 스플리팅 등이 있습니다.",
      },
      {
        id: "react_17",
        question: "React에서 PropTypes는 어떤 역할을 하나요?",
        answer:
          "PropTypes는 컴포넌트가 받는 props의 타입을 검사하여 잘못된 props가 전달될 경우 경고를 표시하는 기능입니다. 이를 통해 코드의 안정성을 높일 수 있습니다.",
      },
      {
        id: "react_18",
        question: "React의 컴포지션(Composition)이란 무엇인가요?",
        answer:
          "컴포지션은 작은 컴포넌트를 조합하여 더 복잡한 컴포넌트를 구성하는 방식입니다. 상속 대신 컴포지션을 통해 코드의 재사용성과 가독성을 높일 수 있습니다.",
      },
      {
        id: "react_19",
        question: "React의 Suspense와 Lazy는 무엇인가요?",
        answer:
          "Suspense와 Lazy는 코드 스플리팅을 통해 컴포넌트를 지연 로드하는 데 사용됩니다. Lazy는 동적으로 컴포넌트를 로드하고, Suspense는 로드 중일 때 로딩 상태를 보여줍니다.",
      },
      {
        id: "react_20",
        question: "JSX란 무엇인가요?",
        answer:
          "JSX는 JavaScript와 HTML을 결합한 문법으로, React에서 UI를 구성할 때 사용됩니다. JSX는 JavaScript 코드로 변환되어 브라우저에서 동작합니다.",
      },
    ],
  };
